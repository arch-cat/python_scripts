...
class LogFormatter(logging.Formatter):
    ...
    def format(self, record, *args, **kwargs):
        if self.color == True and record.levelno in self.COLOR_CODES:  # noqa: E712
...
def set_up_logging(
    ...
    console_formatter = LogFormatter(
        fmt=log_line_template, color=console_log_color, datefmt="%d/%m/%Y %H:%M:%S"
    )
    console_handler.setFormatter(console_formatter)
    logger.addHandler(console_handler)

    try:
        logfile_handler = logging.FileHandler(logfile_file, mode="w+", encoding="utf-8")
